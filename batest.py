...
#     test_df = test_df.sample(n=10)
    ...
    #test_partner_encodings = tokenizer(test_partner_texts, truncation=True, padding=True, max_length=max_length)
    ...
    const_bnn_prior_parameters = {
            ...
            "type": type_rep,  # Flipout or Reparameterization
            "moped_enable": moped_enable,  # True to initialize mu/sigma from the pretrained dnn weights
...
dataset_paths = {
    ...
    'imdb': ('/home/ptrust/experiments/imbalance/imdb_imbalanced.csv', '/home/ptrust/experiments/data/imdb_test.csv'),
    'agnews': ('/home/ptrust/experiments/imbalance/ag_news_imbalanced.csv', '/home/ptrust/experiments/data/train_agnews.csv')
}

results = []

for dataset_name, model_path, num_monte_carlo, prior_mu, prior_sigma, type_rep, moped_enable, moped_delta in itertools.product(
        dataset, model_paths_list, monte_carlo_samples_list, prior_mu_list, prior_sigma_list, type_rep_list, moped_enable_list, moped_delta_list):
    train_path, test_path = dataset_paths[dataset_name]
    f1, predictive_uncertainty, model_uncertainty = run(
        train_path, test_path, model_path, MAXLEN, num_monte_carlo, prior_mu, prior_sigma, posterior_mu_init, posterior_rho_init, type_rep, moped_enable, moped_delta
    )
    results.append({
